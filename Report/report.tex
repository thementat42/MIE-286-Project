...
The dependent variables in this experiment are accuracy, measured as the proportion of correct questions out of 20, and response time in milliseconds (i.e., time taken from seeing the question to clicking an answer).
The independent variable is the type of feedback the participant receives (either gain-based, or loss-based). Participants were randomly assigned to one of these groups to prevent bias via a coin flip.\newline
...
\begin{table}[H]
    \centering
    \caption{Sample mean ($\bar{x}$) and standard deviation $S$ for accuracy ($a$) and response time in ms ($\tau$) for loss-based feedback, gain-based feedback, and baseline trials. "Baseline" refers to the no feedback condition is grouped according to the subsequent feedback condition completed by the participants. $n_{loss}=14$ and $n_{gain} = 16$.}
...
\begin{figure}[H]
    ...
    \caption{Boxplots for Accuracy and Response Time as absolute scores. The plots for performance relative to the baseline, as well as larger versions of these plots, can be found in Appendix \ref{appdx:boxplots}}
    \label{fig:boxplots}
\end{figure}
